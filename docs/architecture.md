...
| Share Processor  | `shareproc`          | The core validation engine. Consumes raw Protobuf shares, validates them statefully, and publishes results. (Kafka Streams) Reconstructs and publishes solved blocks. | `github.com/lovoo/goka`, `github.com/btcsuite/btcd/btcutil`, `google.golang.org/protobuf`               | In-memory state management for job IDs is crucial to avoid DB lookups. Logic must be heavily optimized. |
